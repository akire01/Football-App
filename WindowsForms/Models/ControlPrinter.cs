            ...
            for (int i = parent.Controls.Count - 1; i >= 0; i--)
            {
                ...
                else
                {
                    if ((parent != ctl) && parent is ScrollableControl scrctl)
                    {
                        ...
                        if (parent is ScrollableControl scrcl2)
                        {
                            if (scrcl2.VerticalScroll.Visible) scrContainerSize.Width -= (SystemInformation.VerticalScrollBarWidth + 1);
                            if (scrcl2.HorizontalScroll.Visible) scrContainerSize.Height -= (SystemInformation.HorizontalScrollBarHeight + 1);
                        }
